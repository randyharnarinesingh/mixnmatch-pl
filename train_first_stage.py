...
def define_optimizers(netG, netsD, BD, encoder):
    ...
    for i in range(3):
        if i == 0 or i == 2:
            optimizersD.append(
                optim.Adam(netsD[i].parameters(), lr=2e-4, betas=(0.5, 0.999))
            )
        else:
...
class Trainer(pl.LightningModule):
    def __init__(
        ...
        # losses used throughout training_step
    ...
    def prepare_code(self):
        ...
        free_z = (
            torch.FloatTensor(self.batch_size, cfg.GAN.Z_DIM).normal_(0, 1).to(device)
        )
    ...
    def prepare_data(self, data):
        ...
        real_p = child_to_parent(
            real_c, cfg.FINE_GRAINED_CATEGORIES, cfg.SUPER_CATEGORIES
        ).to(device)
        real_z = (
            torch.FloatTensor(self.batch_size, cfg.GAN.Z_DIM).normal_(0, 1).to(device)
        )
    ...
    def train_Dnet(self, idx):
        ...
        if idx == 0:
            ...
            for i in range(self.batch_size):
                ...
                a2 = (
                    min(
                        torch.tensor(self.n_out - 1).float().to(device),
                        torch.floor(
                            (self.n_out - 1)
                            - ((126 - self.recp_field) - x2) / self.patch_stride
                        ),
                    )
                    + 1
                )
                ...
                b2 = (
                    min(
                        ...
                        torch.floor(
                            (self.n_out - 1)
                            - ((126 - self.recp_field) - y2) / self.patch_stride
                        ),
                    )
                    + 1
                )
                ...
                if x1 != x2 and y1 != y2:
                    weights_real[
                        ...
                        b1.type(torch.int) : b2.type(torch.int),
                    ] = 0.0

            norm_fact_real = weights_real.sum()
            norm_fact_fake = (
                weights_real.shape[0]
                * weights_real.shape[1]
                * weights_real.shape[2]
                * weights_real.shape[3]
            )
            ...
            if norm_fact_real > 0:
                real_prediction_loss = real_prediction_loss * (
                    (norm_fact_fake * 1.0) / (norm_fact_real * 1.0)
                )
            ...
            class_prediction_loss = self.RF_loss_un(
                class_prediction, weights_real
            ).mean()

            # add three losses together
            D_loss = (
                cfg.TRAIN.BG_LOSS_WT * (real_prediction_loss + fake_prediction_loss)
                + class_prediction_loss
            )
    ...
    def train_BD(self):
        ...
        pred_gen_z, pred_gen_b, pred_gen_p, pred_gen_c = self.BD(
        ...
        penalty = cal_gradient_penalty(
            self.BD, real_data, fake_data, device, type="mixed", constant=1.0
        )
    ...
    def train_EG(self):
        ...
        pred_z, pred_b, pred_p, pred_c = self.encoder(
            free_fake_imgs[2].detach(), "logits"
        )
        ...
        bg_rf_loss = (
            self.RF_loss(self.bg_rf_pred, torch.ones_like(self.bg_rf_pred))
            * cfg.TRAIN.BG_LOSS_WT
        )
        bg_class_loss = self.RF_loss(
            self.bg_class_pred, torch.ones_like(self.bg_class_pred)
        )

        # child image real/fake loss
        _, self.child_rf_pred = self.netsD[2](self.fake_imgs[-1])
        child_rf_loss = self.RF_loss(
            self.child_rf_pred, torch.ones_like(self.child_rf_pred)
        )

        # fool BD loss
        pred_enc_z, pred_enc_b, pred_enc_p, pred_enc_c = self.BD(
            self.real_img, self.fake_z, self.fake_b, self.fake_p, self.fake_c
        )
        pred_gen_z, pred_gen_b, pred_gen_p, pred_gen_c = self.BD(
            self.fake_imgs[2], self.real_z, self.real_b, self.real_p, self.real_c
        )
        fool_BD_loss = (
            pred_enc_z.mean()
            + pred_enc_b.mean()
        ...
        EG_loss = (
            (p_code_loss + c_code_loss)
            + (bg_rf_loss + bg_class_loss)
            + child_rf_loss
            + fool_BD_loss
    ...
    def validation_step(self):
        self.code_z, self.code_b, self.code_p, self.code_c = self.encoder(
            self.fixed_image, "softmax"
        )
        self.fake_imgs, self.fg_imgs, self.mk_imgs, self.fg_mk = self.netG(
            self.code_z, self.code_c, self.code_p, self.code_b, "code"
        )
        save_img_results(
            None,
    ...
    def configure_optimizers(self):
        optimizersD, optimizerBD, optimizerGE = define_optimizers(
